...
export default async function LessonPage({params}:Props){
  ...
  return <main className="shell lesson-page"><div className="page-head lesson-head"><Link href={`/curso/${slug}`} className="meta">← Volver al curso</Link><span className="eyebrow">{course.title} / {module.title}</span><h1>{lesson.title}</h1><p className="lead">Reproduce la lección y revisa el material de apoyo.</p></div><section className="lesson-content"><div className="lesson-player">{lesson.video_url?<iframe src={lesson.video_url} title={lesson.title} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen/>:<div className="notice">Esta lección todavía no tiene un video publicado.</div>}</div><article className="card lesson-reading"><span className="eyebrow">Material de lectura</span><h2>Notas de la lección</h2>{lesson.body?<div className="lesson-body">{lesson.body}</div>:<p className="muted">No hay notas adicionales para esta lección.</p>}<form action={completeLesson}><input type="hidden" name="lesson_id" value={lesson.id}/><button>Marcar como completada</button></form></article></section></main>;
}
