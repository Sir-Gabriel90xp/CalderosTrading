...
export default async function PaymentsPage() {
  ...
  const [{ data: transfers }, { data: payments }] = await Promise.all([
    db.from('bank_transfers')
      .select('id,course_id,amount,currency_code,bank,reference,receipt_path,status,created_at,courses(title)')
      .eq('user_id', user.sub)
    ...
    <main className="shell layout">
      ...
      <div className="dashboard stack">
        ...
        <section id="pagos" className="card">
          ...
          {!!transfers?.length && <div className="tablewrap"><table className="table"><thead><tr><th>Curso</th><th>Método</th><th>Monto</th><th>Estado</th><th>Comprobante</th></tr></thead><tbody>{transfers.map((transfer) => { const course = transfer.courses as unknown as { title: string } | null; return <tr key={transfer.id}><td>{course?.title || 'Curso'}</td><td>{transfer.currency_code==='USD'?'PayPal':'Transferencia bancaria'}<br /><span className="meta">{transfer.bank}<br />Ref. {transfer.reference}</span></td><td>{transfer.currency_code==='USD'?'USD':'RD$'} {Number(transfer.amount).toFixed(2)}</td><td><span className={statusClass(transfer.status)}>{statusLabel(transfer.status)}</span></td><td>{receiptUrls[transfer.id] ? <a href={receiptUrls[transfer.id]} target="_blank" rel="noreferrer">Ver comprobante ↗</a> : 'No disponible'}</td></tr>; })}</tbody></table></div>}
