        ...
        {!visibleProfiles.length ? <div className="card community-empty"><span>🌱</span><h3>El ranking empieza contigo</h3><p>Aún no hay perfiles públicos. Completa el tuyo y activa “Mostrar mi perfil en Comunidad” para aparecer aquí.</p><Link className="button" href="/dashboard/perfil">Publicar mi perfil</Link></div> : <>
          <div className="podium-grid">
            {topThree.map((profile, index) => <article className={`podium-card podium-${index + 1}`} key={profile.user_id}>
              ...
              <Link className="podium-avatar" href={`/comunidad/${profile.user_id}`} aria-label={`Ver el perfil de ${profile.display_name}`}>{avatars.get(profile.user_id) ? <Image unoptimized src={avatars.get(profile.user_id)!} width={84} height={84} alt={`Foto de ${profile.display_name}`} /> : <span>{profile.display_name.slice(0, 1).toUpperCase() || 'T'}</span>}</Link>
      ...
      <section id="certificados" className="community-section certificates-community">
        ...
        {!certificates.length ? <div className="certificate-community-empty"><span>🏅</span><p>Cuando un estudiante comparta un certificado, aparecerá aquí.</p></div> : <div className="certificate-community-grid">{certificates.map((certificate) => {
          ...
          return <article className="certificate-community-card" key={certificate.id}>
            ...
            <div className="certificate-community-info"><span className="eyebrow">{certificate.issuer || 'Logro de trading'}</span><h3>{certificate.title}</h3><div className="certificate-author"><Link href={`/comunidad/${certificate.user_id}`} aria-label={`Ver el perfil de ${profile?.display_name || 'Trader'}`}>{avatars.get(certificate.user_id) ? <Image unoptimized src={avatars.get(certificate.user_id)!} width={34} height={34} alt="" /> : <span className="mini-avatar">{profile?.display_name.slice(0, 1).toUpperCase() || 'T'}</span>}</Link><Link href={`/comunidad/${certificate.user_id}`}>{profile?.display_name || 'Trader'} <small>{countryFlags[profile?.country || ''] || '🌎'} {profile?.country || 'Global'}</small></Link>{href && <a className="button ghost" href={href} target="_blank" rel="noreferrer">Ver logro ↗</a>}</div></div>
          </article>;
        })}</div>}
